...
@MainActor final class DockSurface: NSView {
    ...
    private var horizontal: Bool { model.preferences.edge == .bottom }
    ...
    func synchronize() {
        ...
        for (item, button) in utilityButtons {
            button.image = model.utilities.icon(item)
            if item == .desktop {
    ...
    func updatePointer(at location: NSPoint? = nil) {
        ...
        if ProcessInfo.processInfo.environment["EVERYDOCK_DEBUG_POINTER"] == "1", point != lastDiagnosticPoint {
        ...
        window.ignoresMouseEvents = !onDock
    ...
    @objc private func tick() {
        ...
        let interpolation = reduceMotion ? 1.0 : 1 - exp(-max(1.0 / 120, min(0.05, now - lastTick)) * 20)
        lastTick = now
        let length = horizontal ? bounds.width : bounds.height
        let allIDs = visibleApps.map(\.id) + DockUtility.allCases.map { "utility.\($0.rawValue)" }
        ...
        for (index, id) in allIDs.enumerated() {
            ...
            let target = pointerAxis.map { DockMotion.scale(distance: $0 - center, iconSize: baseSize,
                                                           magnification: model.magnification) } ?? 1
        ...
        glass.frame = backgroundRect
        glass.cornerRadius = thickness * 0.28
        indicators.frame = bounds
        ...
        if let hovered, let button = buttons[hovered.id] {
            tooltip.stringValue = hovered.name + (hovered.isLaunching ? " — 실행 중…" : "")
            let width = tooltip.intrinsicContentSize.width + 18
            let height = 26.0
            switch model.preferences.edge {
            case .bottom:
    ...
    func stop() { animationTimer?.invalidate(); animationTimer = nil }
...
private final class DockIndicators: NSView {
    ...
    override func draw(_ dirtyRect: NSRect) {
        for (rect, active) in dots {
            (active ? NSColor.labelColor : NSColor.secondaryLabelColor).setFill()
...
@MainActor final class DockAppButton: NSButton {
    ...
    override func draw(_ dirtyRect: NSRect) {
        image?.draw(in: bounds, from: .zero, operation: .sourceOver, fraction: cell?.isHighlighted == true ? 0.6 : 1,
...
@MainActor final class DockUtilityButton: NSButton {
    ...
    init(item: DockUtility, model: AppModel) {
        ...
        image = model.utilities.icon(item)
        toolTip = item.title
