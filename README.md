...
- 우클릭 메뉴의 **앱의 macOS Dock 메뉴…** 또는 **Option-우클릭**은 카카오톡 등 앱이 제공하는 실제 Dock 메뉴를 엽니다. 손쉬운 사용 권한과 기본 Dock의 앱 항목이 필요하며 메뉴 위치는 macOS가 결정합니다.
...
자동 테스트 36개는 좌표·화면 제한, 확대 곡선, 설정 이전, 유틸리티를 포함한 치수, 요술램프 설정 복원, 사용자 변경 보존을 검증합니다. 앱 감지는 실행 알림과 KVO를 사용하고 백그라운드 5초 확인을 보완합니다. 상태가 같으면 앱 목록을 다시 발행하지 않습니다. 확대는 화면에 연결된 CADisplayLink와 캐시된 아이콘 레이어를 사용하며, 안정되면 프레임 갱신을 멈춥니다. 실제 기기 검증과 미검증 항목은 [QA.md](QA.md)에 기록합니다.
