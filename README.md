...
- 우클릭 메뉴의 **시스템 Dock 메뉴… (기본 Dock 위치)** 또는 **Option-우클릭**은 카카오톡 등 앱이 제공하는 실제 Dock 메뉴를 엽니다. 손쉬운 사용 권한과 기본 Dock의 앱 항목이 필요하며 메뉴 위치는 macOS가 결정합니다.
...
시스템 Dock 메뉴는 기본 Dock이 같은 모니터에 있을 때만 요청합니다. 다른 모니터에서는 잘못된 위치에 띄우지 않고 안내합니다. 앱 고유 메뉴를 모든 everyDock 아이콘 위치에 복제하는 기능은 지원하지 않습니다.
