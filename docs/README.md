...
| MR-06 사용자가 제어 | P-06 | FR-16~FR-18, FR-26~FR-27 | TC-A04, A06, A40~A41, A44~A45, M28~M33, M53~M54, M56~M58 |
...
현재 기본 자동 검사는 42개 PASS 대상과 명시적 Quick Look 통합 검사 1개입니다. 통합 검사를 켜면 총 43개를 실행합니다. 그중 `DockLayout`을 호출하는 3개는 레거시 순수 좌표 함수 검증이며, 실제 `DockCoordinator`의 화면 배치를 검증하는 UI 테스트가 아닙니다. 기존 실기기 기록은 [QA.md](../QA.md)에 보존합니다. 성능 목표와 미허용 권한 경로는 별도 검증이 필요합니다.

현재 바이너리는 ad-hoc 서명된 초기 배포본입니다. Developer ID 공증, 권한을 허용한 실제 최소화·캡처·파일 처리, 광범위한 모니터 조합 검증이 끝나기 전에는 안정 버전으로 표시하지 않습니다.

코드와 문서는 [AGENTS.md](../AGENTS.md)의 정합 규칙을 따른다. 최신 수정·실행 근거는 [v0.3.7 QA](QA-v0.3.7.md)에 기록한다. 과거 버전 PASS는 새 버전 전체 검증을 뜻하지 않는다.
