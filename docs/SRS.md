...
선택된 `NSScreen`마다 안정적인 디스플레이 식별자를 키로 패널을 하나만 유지해야 한다. 화면 변경·Spaces 변경·시스템 및 화면 깨어남을 관찰하고, 연결 해제된 패널과 미리보기·타이머를 정리한다. 음수 원점과 포인트 좌표를 지원해야 한다. 1초 주기의 geometry 확인은 이벤트 누락을 보완한다.
...
길이는 `(앱 수 + 3) × (아이콘 크기 + 2) - 2 + 14 + 12`다. 이는 everyDock의 구현 기준으로, Apple 내부 치수를 측정한 공식 값이 아니다. 아이콘은 NSButtonCell 추가 패딩 없이 전체 아이콘 bounds에 그린다. 좁은 화면에서는 크기를 줄이고 목록 스크롤을 제공한다.

### FR-04 모션과 입력 — P-02 / P0

포인터 거리에 따른 연속 cosine 감쇠로 중심과 이웃 아이콘을 확대해야 한다. 변화 중에만 60Hz 타이머를 돌리고 안정되면 멈춘다. 실행 바운스와 클릭 반응을 제공하고, 동작 줄이기 시 이동 효과를 줄인다. 이벤트 좌표와 실제 포인터 좌표를 올바르게 변환하고 투명 애니메이션 공간의 클릭은 아래 앱으로 통과시킨다.
...
활성 앱 재클릭 옵션이 켜져 있으면 AX focused window의 minimize button에 Press를 요청하고, 지원하지 않으면 minimized 속성을 설정한다. 최소화 창 복원 시 minimized=false 및 raise를 요청한다. 현재 구현은 앱 클릭 복원 시 앱의 최소화 창들을 복원하며, 개별 창 선택은 FR-15를 사용한다.

AX 권한이 없거나 대상이 거부하면 앱 숨김으로 위장하지 말고 안내해야 한다. AX messaging timeout은 0.2초로 설정하고 UI 스레드 밖에서 호출한다. 시스템이 요술램프 효과와 도착 위치를 결정하며 everyDock별 도착점 지정은 범위 밖이다.

### FR-07 앱 상태 일관성 — P-02 / P0

실행·종료·활성화·숨김 알림과 KVO를 주 경로로 사용하고 0.5초 재확인으로 보완해야 한다. pending 요청은 최대 45초 보관한다. 앱 목록·순서·실행/활성/숨김/launching 상태가 같으면 목록을 재발행하지 않는다. 포커스 변화만으로 앱 순서가 바뀌면 안 된다.
...
### FR-10 바탕화면 토글 — P-04 / P1

숨기기 요청 **전에** 현재 보이는 regular 앱 목록과 활성 앱을 저장해야 한다. 다시 클릭하면 저장한 종료되지 않은 앱만 복원하고 이전 활성 앱으로 돌아간다. 숨김 요청 반환값을 최종 상태로 간주해 복원 목록을 잃으면 안 된다. 정상 종료 시 숨겼던 앱을 복원한다. 강제 종료 시 이 목록의 영속 복원은 현재 범위 밖이다.

### FR-11 다운로드 스택 — P-04 / P1

사용자의 Downloads 폴더에서 숨김 항목을 제외하고 수정일 역순 최대 80개를 4열로 표시해야 한다. 클릭하면 기본 앱으로 열며 Finder 열기를 제공한다. 로딩·비어 있음·실패·접근 응답 대기 상태를 구분한다. 2초 이상 대기하면 접근 안내를 표시한다. 팝업을 반복 개방해도 진행 중 디렉터리 요청은 앱당 하나만 유지한다.

### FR-12 휴지통 — P-04 / P1

현재 사용자 `~/.Trash`를 열고 내용 변화에 따라 빈/찬 아이콘을 갱신해야 한다. 비우기는 대상 범위·영구 삭제·취소 불가를 알리고 명시적 확인 후 수행한다. 취소가 기본 키 응답이어야 하며 취소는 아무 항목도 삭제하지 않는다. 외장 드라이브의 휴지통은 포함하지 않는다. 읽기·삭제 실패는 오류로 표시한다.
...
팝업이 열린 동안 약 2초 간격 갱신한다. 캐시는 최근 32개, 조회 시 60초 넘은 항목을 정리하는 메모리 캐시다. 앱 종료 시 소멸한다. 화면 기록 미허용·캡처 실패·보호된 창은 안내 또는 제목/대체 이미지로 대응한다. 공개 API가 제공하지 않는 창 캡처를 우회하지 않는다.
...
AX 신뢰·화면 기록 허용 상태를 표시하고 사용자가 시스템 설정을 열 수 있어야 한다. 기능별 필요 이유를 설명하며 승인 자체는 OS에서 사용자가 수행한다. ad-hoc 재빌드/업데이트 후 권한 재등록 가능성을 설치 문서에 알린다. 거부 상태에서도 기본 앱 실행·설정·종료를 제공한다.
...
| NFR-04 | 개인정보 | 네트워크·분석 전송 없음. 창 이미지 파일 저장 없음. 캡처 권한 없으면 이미지 캡처 요청 안 함 | 코드 확인, 런타임 관찰 필요 |
...
| NFR-08 | 추적 가능성 | 모든 P 요구에 FR/NFR과 TC 연결, PASS에 날짜·환경·근거 명시, 제약 공개 | 문서 검토 대상 |
