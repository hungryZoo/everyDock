...
길이는 `(앱 수 + 3) × (아이콘 크기 + 2) - 2 + 14 + 12`다. 이는 everyDock의 구현 기준으로, Apple 내부 치수를 측정한 공식 값이 아니다. 아이콘은 NSButtonCell 추가 패딩 없이 bounds에 맞춘 CALayer에 표시한다. NSImage가 변경될 때만 CGImage를 만들며 확대 중 재래스터화하지 않는다. 표시 점·구분선도 재사용 레이어다. 좁은 화면에서는 크기를 줄이고 목록 스크롤을 제공한다.
