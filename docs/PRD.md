...
2026-09-10 추가 인수: P-02는 추가 4pt 틈 제거와 관찰한 원래 크기로 반복 확대/복원을 포함한다. 앱 고유 명령은 실제 macOS Dock 메뉴 열기로 접근하며 시스템이 메뉴 위치를 결정한다. P-04 파일 팝업은 수정일 최신순 표시와 비동기 내용 썸네일을 제공한다. 미지원 파일은 아이콘을 유지하고 팝업을 닫으면 작업을 취소한다. 구현 상세는 FR-22~FR-25와 TC-M43~TC-M47에 연결한다.

P-02 복원 전환은 화면 동기화된 짧은 위치·크기 애니메이션을 제공한다. 동작 줄이기를 존중하고 일반 Dock 호버 렌더링을 막지 않는다. 실제 부드러움은 자동 좌표 테스트와 별도로 검증한다.

앱 고유 시스템 메뉴는 기본 Dock과 같은 화면에서만 요청하는 제한 기능이다. 다른 화면에서는 실행을 차단하고 everyDock 메뉴를 유지한다. 사용자가 보고한 다른 모니터 표시 문제를 성공으로 처리하지 않는다.
